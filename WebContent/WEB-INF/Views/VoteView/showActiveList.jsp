...
<!-- 列表开始 -->
<div class="smallList">
   <c:forEach items = "${activelist}" var = "voteActivity" >
       <a class="S-item" href="../show/activity?voteId=${voteActivity.voteId}">
          <div class="S-itemContent">
               <fmt:formatDate pattern="yyyy-MM-dd' 'HH:mm" value="${voteActivity.startDate}" var="startDate"/>
               <div class="S-itemText-T"><c:out value="${voteActivity.voteTitle}"/></div>
               <div class="S-itemText-D"><c:out value="${startDate}"/></div>
          </div>
       </a>
   </c:forEach>
</div>
