...
<c:forEach items = "${itemlist}" var = "voteItem" >
	...
	<input type="radio" id="${voteItem.itemId}" name="rr" value='${voteItem.itemId}'/>
	<label for="${voteItem.itemId}"></label>
	</div>
</div>  
</c:forEach>
<button class="button94" onclick="saveChoice('${openId}')">提交</button>
</div>
<!-- 详情结束（单选） -->
</div>
...
function saveChoice(openId){
	var choice = new Object();
	choice.openId = openId;
	var radios = document.getElementsByName('rr');
	for (var i = 0, length = radios.length; i < length; i++) {
	    if (radios[i].checked) {
	        choice.itemId = parseInt(radios[i].value, 10);
	        break;
	    }
	}
	var url = getRootPath() + "/vote/user/submit/choice/single";
	if(choice.itemId == null){
		alert('Please select a choice!');
	}else{
		sendAjax(JSON.stringify(choice), url, checkResult);
	}
}
